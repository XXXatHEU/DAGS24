...
const (
	...
	ResponeFetchBlocks //响应请求数据
	FetchTX
	Productcomm //广播product
	Pullok      //来拉取我的区块啊
)

type DBfileMessage struct {
	DB            []byte // 数据库
	lastBlockHash []byte // 最后一个区块的哈希值
...
func startConnWork(conn net.Conn) {
	...
	getmsg := make(chan []byte, 50)
	go GetSendTaskfromPool(Down, getmsg)
	recvmsgChan := make(chan *SendMessage, 50)
	//收到请求
	...
	for {
		...
		//发送线程  getmsg将发送请求
		case msg := <-getmsg:
			Log.Debug("startConnWork已经被唤醒，触发发送")
			err := sendRequest(conn, msg)
			if err != nil {
				Log.Warn("广播中本节点发送失败,将断开本连接:", err)
				DelDialGoPool(index)
				return
			} else {
				ip, port := ReturnIpwithPort(conn)
				Log.Debug("向", ip, "：", port, " 发送信息成功")
			}
		case recvResMsg := <-recvmsgChan:
			//Log.Debug(recvResMsg)
			switch recvResMsg.Cmd {
			case SendTX:
				Log.Info("接收到发来的交易：SendTX")
				ReceiveTxArr(recvResMsg)
				//这个是收到请求更新的区块
			case SendCommonBlock: //这个情况我好像没写实现
				Log.Info("接收到主动发起请求的回应，将对比或将更新本区块")
				ReceiveBlockArr(recvResMsg)
			case SendMinedBlock: //这个是收到挖矿的区块
				Log.Info("接收到MinedBlock广播的区块")
				ReceiveMinedBlock(recvResMsg, getmsg)
			case SendGenesisBlock:
				Log.Info("接收到SendGenesisBlock")
			case FetchBlocks:
				Log.Warn("收到请求获取区块进行同步的命令==========================")
				FetchBlocksResponse(getmsg)
			case ResponeFetchBlocks:
				Log.Warn("对方发来区块链文件==========================")
				GetFetchBlocksResponse(recvResMsg, getmsg)
			case Productcomm:
				Log.Info("收到对方发来的产品信息")
				ReceiveProduct(recvResMsg)
			case Pullok: //他想让我主动请求他
				Log.Warn("他想让我主动请求他=========================")
...
func startDialing(peerAddr string, Wg *sync.WaitGroup) {
	defer Wg.Done()
	if peerAddr == addStr {
		Log.Debug("跳过连接自己节点的过程")
		return
	}
	//需要判断是否已经连接 根据已连接的数组来判断
	hasConnAddrsMu.Lock()
	Log.Debug("打印已经连接的节点信息")
	for key, value := range hasConnAddrsMap {
		Log.Debug(key, value)
	}
	_, exists := hasConnAddrsMap[peerAddr]
	if exists {
		Log.Debug("连接已经建立，将退出本次建立过程")
		hasConnAddrsMu.Unlock()
		return
	}
	/*
	...
	*/
	...
	Log.Debug("已连接到", conn.RemoteAddr())
