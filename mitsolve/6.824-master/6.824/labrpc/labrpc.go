...
type Service struct {
	...
	methods map[string]reflect.Method // 方法集合，以方法名为键，对应的反射方法为值
}

// MakeService 函数用于创建一个 Service 类型的对象。
// 它接收一个接收器(rcvr)作为参数，并返回构建好的 Service 对象。
func MakeService(rcvr interface{}) *Service {
	svc := &Service{}
	svc.typ = reflect.TypeOf(rcvr)                      // 获取接收器的反射类型
	svc.rcvr = reflect.ValueOf(rcvr)                    // 获取接收器的反射值
	svc.name = reflect.Indirect(svc.rcvr).Type().Name() // 获取接收器的名称
	svc.methods = map[string]reflect.Method{}           // 初始化方法集合
	...
	for m := 0; m < svc.typ.NumMethod(); m++ {
		...
		// 检查方法是否适合作为处理程序(handler)
		if method.PkgPath != "" || // 首字母大写?
			mtype.NumIn() != 3 || // 参数数量是否为3个?
			//mtype.In(1).Kind() != reflect.Ptr || // 第一个参数是否为指针类型?
			mtype.In(2).Kind() != reflect.Ptr || // 第二个参数是否为指针类型?
			mtype.NumOut() != 0 { // 输出参数数量是否为0?
			// 该方法不适合作为处理程序
			//fmt.Printf("bad method: %v\n", mname)
		} else {
			// 该方法符合处理程序的要求，将其添加到方法集合中
