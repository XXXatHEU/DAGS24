...
for ((i=0; i<${#new_values[@]}; i++)); do
  ...
  sed -i "s/$variable_name1 = [0-9]\+/ $variable_name1 = $new_value/" $prefolder_path/miningMain.go
  sed -i "s/$variable_name2 = [0-9]\+/ $variable_name2 = $new_value/" $prefolder_path/miningMain.go
  ...
  if [ ! -d "$folder_path" ]; then
    mkdir -p "$folder_path"
    cp $prefolder_path/* "$folder_path" -r
    #cp $prefolder_path/wallet/* ./
  fi
