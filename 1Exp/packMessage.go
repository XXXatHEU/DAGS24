...
func PackTxArrTaskAndToChan(cmd Command, txs []*Transaction) {
	...
	var cmdBytes [4]byte
	binary.BigEndian.PutUint32(cmdBytes[:], uint32(cmd))
	Log.Debug("cmd:", uint32(cmd))
	buf = append(cmdBytes[:], buf...)

	// 序列化 Len 字段
	var lenBytes [4]byte
	binary.BigEndian.PutUint32(lenBytes[:], uint32(len(buf)+4)) //这里并没有加入自己的4个长度
	Log.Debug("消息长度为：", uint32(len(buf)+4))
	buf = append(lenBytes[:], buf...)
	//fmt.Println(buf)
	// 将 buf 发送到消息队列中
...
//打包命令的函数
func PackCommondAndToChan(cmd Command, getmsg chan []byte) {
	...
	if getmsg == nil {
		SendtaskChan <- buf
		//Log.Fatal("逻辑发生错误，如果想要单人广播，必须要传入一个通道过来。错误位于PackCommondAndToChan")
	} else { //否则单独进行
		getmsg <- buf
	}
...
//打包DBfileMessage类型的数据的函数   注意这里不再发送到chan里面
func PackDBfileMessageTaskAndToChan(cmd Command, dbfileMessage DBfileMessage, getmsg chan []byte) {

	// 序列化 Payload 字段
	payloadBuf := new(bytes.Buffer)
	err := binary.Write(payloadBuf, binary.BigEndian, DBfileMessageToByte(dbfileMessage))
	if err != nil {
		Log.Error("序列化 Payload 失败：", err)
		return
	}
...
func PackProductAndToChan(cmd Command, product *ProductType) {
	// 序列化 Payload 字段
	payloadBuf := new(bytes.Buffer)
	err := binary.Write(payloadBuf, binary.BigEndian, productToByte(product))
	if err != nil {
		Log.Error("序列化 Payload 失败：", err)
		return
	}
...
func productToByte(product *ProductType) []byte {
	var buffer bytes.Buffer

	// 注册需要编码的类型
	gob.Register(&ProductType{})
...
// 将DBfileMessage元素转换为byte数组
func DBfileMessageToByte(dbfile DBfileMessage) []byte {
	var buffer bytes.Buffer

	// 注册需要编码的类型
	gob.Register(&Block{})
	gob.Register(&Transaction{})
	gob.Register(&DBfileMessage{})
...
// 将byte数组恢复为DBfileMessage
func byteToDBfileMessage(byteArr []byte) *DBfileMessage {
	var dbfileMessage *DBfileMessage
	...
	err := decoder.Decode(&dbfileMessage)
	if err != nil {
		Log.Warn("DBfileMessage解码出现问题")
	}

	return dbfileMessage
}

//将product byte反序列化为prodcut
func byteToproduct(byteproduct []byte) *ProductType {
	...
	// 创建解码器
	buffer := bytes.NewBuffer(byteproduct)
	decoder := gob.NewDecoder(buffer)
