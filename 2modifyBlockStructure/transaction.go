...
type TXOutput struct {
	...
	Value            float64 //转账金额
	TraceID          string  //新添加的溯源id
}

//由于没有办法直接将地址赋值给TXoutput，所以需要提供一个output的方法
//func newTXOutput(address string, amount float64) TXOutput {
func newTXOutput(address string, amount float64, TraceID string) TXOutput {
	output := TXOutput{TraceID: TraceID, Value: amount}
...
// # 创建挖矿交易
func NewCoinbaseTx(miner /*挖矿人*/ string, data string, TraceID string) *Transaction {
	...
	output := newTXOutput(miner, reward, TraceID)
...
//创建普通交易
// 1. from/*付款人*/,to/*收款人*/,amount输入参数/*金额*/  TraceID 交易id
func NewTransaction(from, to string, amount float64, TraceID string, bc *BlockChain) *Transaction {
	...
	if !ok {
		fmt.Println("没有找到来源方地址对应的私钥!")
		return nil
	}

	fmt.Println("找到来源方的私钥和公钥，准备创建交易...")
