...
const (
	     BlockhasBlnumMin = 128     //定义一个区块里面最小的交易数量
	     BlockhasBlnumMax = 128     //最大交易数量
	TxPacketPoolMin  = 0     //只有超过一定数量才会通知
	MaxBlockLen      = 10000 //最大挖矿长度
...
func RemoveTxpooltxAndtrystop(block *Block, isInterrupt bool, forceInterrupt bool) {
	...
	if (Interrupt || forceInterrupt) && MiningControlExist {
		if tempSTOP == false {
			Log.Debug("监听组发现挖矿控制协程存在，发送停止挖矿命令")
...
func getTxFromTxPool(ctx context.Context) (dd map[string]*Transaction, verifytxsmap map[uuid.UUID][]*Transaction) {
	for {
		select {
		case <-ctx.Done():
			Log.Debug("getTxFromTxPool发现已经取消，Operation cancelled")
			return nil, nil
		default:
			...
			if tryLock(&TxPoolmutex, 100*time.Millisecond) {
				...
				if len(TxPool) < BlockhasBlnumMin {
					...
					for _, value := range TxPool {
						...
						/*arr := []*Transaction{value}
						flag++
						verifytxsmap[value.TXOutputs[0].SourceID] = arr*/
					}
					//设置uuid正在使用
...
func MinningPMC(ctx context.Context) {
	...
	txs, verifytxsmap := getTxFromTxPool(ctx)
	//将map转换为切片
	transactionSlice := make([]*Transaction, 0, len(txs))
...
func saveConfirmedSideBlock(block Block) {
	Log.Debug("开始存入侧链区块")
	createOrSaveTxs(block.Transactions)
	Log.Debug("存入侧链区块完成")
