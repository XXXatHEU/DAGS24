...
func (cli *CLI) addBlock(data string) {
	cli.addBlockChainopp("手动增加区块:" + data)
}
...
func (cli *CLI) createBlockChain(productmsg string) {
	...
	if err != nil {
		Log.Debug("CreateBlockChain failed:", err)
		return
	}
...
func printBlock(block Block) {
	...
	for uid, lists := range block.ConfirmedLists {
		...
		Log.Debug("ConfirmedLists验证交易区块:%s\n", uid.String())
...
func (cli *CLI) printFromMemory() {
	...
	for {
		...
		fmt.Printf("	----------------------区块自有Product\n")
		fmt.Printf("		绑定溯源sourceid:\n			%s\n", string(block.Product.SourceID.String()))
		fmt.Printf("		绑定溯源pubkeyhash:\n			%x\n", block.Product.Pubkeyhash)
		fmt.Printf("		绑定溯源source的txid:\n			%x\n", block.Product.StartTxid)
		address, err := pubkeyhashToAddress(block.Transactions[0].TXOutputs[0].ScriptPubKeyHash)
		if err == nil {
			fmt.Printf("		本地对区块解析后，product归属账户地址:\n			%s\n", address) //矿工写入的数据
		} else {
		...
		fmt.Printf("	----------------------\n")
		//通过验证交易区块
		fmt.Println("长度为================ ", len(block.Transactions))
		fmt.Println("长度为================ ", len(block.ConfirmedLists))
		for uid, lists := range block.ConfirmedLists {
			//fmt.Println("长度为================ ", len(block.ConfirmedLists))
			fmt.Printf("验证交易区块:%s\n", uid.String())
			...
			fmt.Printf("修改内容 添加额外信息：%x\n", lists.FinalBelongPubKeyHash)
		}
		...
		for _, confirmedBlockList := range block.ConfirmedLists {
			...
			for _, confirmedBlockType := range blockArray {
				...
				//gettx := GetTxFromMemory(confirmedBlockType.Hash)
				//printtx(gettx)
			}
		}

		// hashget, _ := getValueByString(hashpre)
		// //2.反序列化并加入到内存中
		// for txhash, _ := range hashget {
		// 	printtx(MemoryTxMap[txhash])
		// }
...
func (cli *CLI) getBalance(address string) {
	...
	for _, utxo := range utxoinfos {
		if flag == 0 {
			Log.Debug(address, "拥有的货物为:", utxo.TXOutput.SourceID)
			flag++
		} else {
			Log.Debug(utxo.TXOutput.SourceID)
		}
		//这两种方式都可以使用
		// total += utxo.Value
	...
	for {
		...
		for i, tx := range block.Transactions {
			...
			if i == len(block.Transactions)-1 {
				fmt.Println("		 主链区块交易，无输入账户！")
			} else {
				inPubkeyhash := getPubKeyHashFromPubKey(tx.TXInputs[0].PubKey)
				inAddress, err := pubkeyhashToAddress(inPubkeyhash)
				if err == nil {
					fmt.Printf("		原先账户地址:\n			%s\n", inAddress) //矿工写入的数据
				} else {
					fmt.Printf("		本地未能解析product归属公钥哈希归属地址，ScriptPubKeyHash:\n			%x\n", inPubkeyhash) //矿工写入的数据
				}
			}
			outAddress, err := pubkeyhashToAddress(tx.TXOutputs[0].ScriptPubKeyHash)
			if err == nil {
				fmt.Printf("		现归属账户地址:\n			%s\n", outAddress) //矿工写入的数据
			} else {
				fmt.Printf("		本地未能解析product归属公钥哈希归属地址，ScriptPubKeyHash:\n			%x\n", block.Transactions[0].TXOutputs[0].ScriptPubKeyHash) //矿工写入的数据
