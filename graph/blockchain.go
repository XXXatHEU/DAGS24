...
func addBlockChain1(address string, productmsg string) error {
	...
	fmt.Printf("NewCoinbaseTx address================= %x\n", getPubKeyHashFromAddress(address))
	//2. 拼装txs, txs []*Transaction
	txs := []*Transaction{coinbase} //最终没有用这个
	//3. 创建创世块
...
func (it *Iterator) Next() (block *Block) {
	...
	if err2 != nil {
		fmt.Printf("iterator next err:", err2)
		return nil
	}
	...
	/*	err := it.db.View(func(tx *bolt.Tx) error {
				...
				blockTmpInfo := bucket.Get(it.currentHash) //一定要注意，是currentHash
	...
	if ok2 {                                      //输入在交易池中
		...
		if bc.verifyTransaction(tx) && bc.verifyHaveSourceid(tx, usedTxto) {
			...
			*count++
			...
			return
		} else if bc.verifyTransaction(tx) { //没有验证通过就删除
			Log.Debug("交易已经使用了，判定为无效交易")
...
func (bc *BlockChain) verifyTransaction(tx *Transaction) bool {
	Log.Debug("正在区块链上检查交易...")

	//根据传递进来tx得到所有需要的前交易即可prevTxs
	//prevTxs := make(map[string]*Transaction)
	id := tx.TXOutputs[0].SourceID
	address := getPubKeyHashFromPubKey(tx.TXInputs[0].PubKey)
	Log.Debug("得到转账人的公钥哈希")
	exit, returnerr := hasAmount(id, address)
	Log.Debug("verifyTransaction hasAmount")
	if returnerr != nil {
		Log.Info("验证失败")
		return false
	}
	if exit {
		Log.Debug("验证通过，将verify验证公钥")
		return true
