...
func ReceiveMinedBlock(message *SendMessage, getmsg chan []byte) {
	...
	Log.Info("进入ReceiveBlockArr处理收到的区块")
	...
	for index, block := range blockarry {
		...
		for _, tx := range block.Transactions {
			//直接打印交易
			fmt.Println(tx.TXOutputs[0].SourceID)
			if !CompareBlock(tx) {
				fmt.Printf("区块中使用的过期的交易，退出更新")
				return
			}
		}
	}
	...
	if fileexit && block.PrevHash == nil {
		Log.Info("发现创世块广播，更新本地")
		//先看满足条件与否，满足则立马打断挖矿
		// DBmutex.Lock()
		// if isFileExist(blockchainDBFile) == true {
		// 	errre := os.Remove(blockchainDBFile)
		// 	if errre != nil {
		// 		Log.Fatal("删除文件出错")
		// 	}
		// }
		// errcr := os.WriteFile(blockchainDBFile, nil, 0644)
	...
	if !exit {
		...
		if myLen >= hisLen { //如果我到那个位置更多，那么抛弃他发来的数据
			...
			FetchBlocksRequest(getmsg) //让他来请求我
		} else {
			//更新交易池
			count := 0
			for _, block := range blockarry {
